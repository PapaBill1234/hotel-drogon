...
Checked 2026-09-26 against code baseline `e014caeb1e47314b0199421d37294f3602add768` and [green CI run 36221358776](https://github.com/PapaBill1234/hotel-drogon/actions/runs/36221358776); the current Docker input change is verified locally in the [measurement report](phase2b-build-loop-measurements.md). This file tracks the Drogon/React application. The former 50 KB inventory, including the Laravel attempt and the investigation diary, is preserved verbatim in [the v1 archive](archive/phase1-parity-inventory-v1.md) and in Git history. The [stable plan](cpp-drogon-conversion-plan.md) owns architecture and safety rules; [run state](ai-run-state.md) owns the active unit.
...
| --- | --- | --- | --- |
| C++ backend, worker binary, MariaDB, Redis, nginx | **Verified for existing scope.** Pinned Ubuntu 24.04/Drogon 1.8.7 build; sanitizers, `-Werror`, CTest, live integration, and isolated cutover passed after updating curl to `8.5.0-2ubuntu10.15` in both Dockerfile stages. | `Dockerfile`, `CMakeLists.txt`, `compose.yaml`; [run 36173602579](https://github.com/PapaBill1234/hotel-drogon/actions/runs/36173602579) on `cf38fdf` passed all three jobs. The strict pin checker was not changed. | Reproducible frontend delivery and truthful worker health remain. Fresh-clone asset acquisition is external. |
| Development and verification loop | **Measured for this host and code baseline.** Cold/warm clean C++ builds took 41.82/37.07 s, one `.cpp` edit 7.07 s, shared-header edit 26.53 s, and each link under 0.2 s. A docs-only Docker rebuild fell from 38.82 to 2.86 s after filtering non-build inputs. | [Phase 2b measurements](phase2b-build-loop-measurements.md); `.dockerignore` keeps docs, frontend dependencies, generated files, and local state out of the backend context. The filtered image, CTest, sanitizer build, security lints, isolated smokes, browser flows, and cutover were checked locally. | Add the Compose frontend build service and truthful worker health; no new build tool is justified by this single baseline. |
...
| 2b | **Incomplete:** the build loop is measured and Docker inputs are filtered; Compose frontend build and truthful worker health remain. All three CI jobs passed on the prior code baseline `e014cae`; the current local verification is recorded in the measurement report. |
...
**Exact next work unit:** make Compose build the React bundle that nginx serves without a host `npm run build`, and give the worker a truthful health state by disabling the inherited HTTP probe or adding a real worker check. Verify from an isolated fresh checkout or equivalent clean project with the declared read-only legacy asset prerequisite, then rerun the Phase 2b image, sanitizer, CI, integration, browser, and cutover checks. Keep primary data intact. The Phase 1 OpenAPI slice remains a separate unmet exit condition after this delivery work.
