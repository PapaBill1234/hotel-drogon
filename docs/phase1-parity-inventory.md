...
| --- | --- | --- | --- |
| C++ backend, worker binary, MariaDB, Redis, nginx | **Verified for existing scope.** Pinned Ubuntu 24.04/Drogon 1.8.7 build; sanitizers, `-Werror` and CTest in CI. | `Dockerfile`, `CMakeLists.txt`, `compose.yaml`; [CI run 35539158338](https://github.com/PapaBill1234/hotel-drogon/actions/runs/35539158338) on `84c8257`: all three jobs succeeded. | No fresh-checkout exercise or measured incremental loop yet. |
...
| 2b | **Incomplete:** Compose frontend build and truthful worker health remain; current code builds/tests in CI and one-route cutover/rollback has passed. |
...
**Exact next work unit:** measure and record cold/warm clean backend builds, a single-`.cpp` rebuild, a widely included-header rebuild, each executable link, a docs-only `docker compose build`, the frontend build, and the three CI job durations. Record environment, commands, wall times, and Docker build inputs. If the audit demonstrates avoidable context invalidation, make a small `.dockerignore` or narrower `COPY` change; verify the image, Catch2/sanitizers, security lints, integration/browser checks, and CI before claiming a gain. Do not install a linker, compiler, cache, PCH, watcher, or other build tool without measured justification and the plan's dependency decision. Use an isolated disposable stack for any reproduction; never reset the primary data.
