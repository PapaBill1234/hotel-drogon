...
| Sentry / Metrics | Before public cutover | **Metrics done; Sentry deferred to a Phase 10 decision gate.** The `/metrics` Prometheus endpoint is live. `AppConfig` reads `SENTRY_DSN` into `cfg.sentry_dsn`, but no Sentry SDK is linked and nothing is ever reported — the field is inert, and no Sentry C++ SDK exists in the Ubuntu 24.04 archive. Recorded as an open gap, not a pass. |

### Phase 2b — build, containerize, verify (status as of the dependency-strategy commit)

The plan's Phase 2b exit condition has four parts. Three are met and the fourth is
partially met: the only remaining implementation item is the Compose frontend
build service. **Phase 2b is still not complete**, and nothing below should be
...
| CI covers TypeScript build and Playwright | **Yes — verified green, but not on the current head.** `integration-smoke` builds the frontend, reloads the proxy, runs the admin UI suite on the runner's Chromium and the parity suite in the pinned container. All steps passed in runs `35530042653` and `35530699981`. (The first attempt failed because `frontend/dist` is a bind-mount source Docker auto-creates as **root**, so Vite's output-directory emptying hit EACCES; fixed by committing `frontend/dist/.gitignore` and reloading rather than restarting the proxy.) |
| **CI passes** | **GREEN on the preceding head, RED on the current one.** Runs `35530042653` and `35530699981` passed with all three jobs success. The dependency-pinning commit then failed twice at "Install system dependencies" (runs `35536762317`, `35537686018`); the cause is identified and fixed — the runner image adds third-party apt repositories (`apt.postgresql.org`), which outrank Ubuntu's `libpq-dev` pin — but the fix has not yet been re-observed on CI. |
...
| Sentry wired | **No — moved to a Phase 10 observability decision gate.** `/metrics` is implemented. No Sentry C++ SDK exists in the Ubuntu 24.04 archive, and a measured x64-linux `sentry-native` vcpkg probe failed building `libunwind` (which does not establish that the port is universally broken). `cfg.sentry_dsn` remains inert. Recorded as a deferred gap, not a pass. |
...
| Dependency strategy chosen and explained | **DONE (plan amended 2026-09-21).** Pinned Ubuntu 24.04 archive packages, base image pinned by digest, direct dependencies pinned by version. Both vcpkg and Conan now resolve Drogon 1.9.13, against which this code does not pass its required `-Werror` build; supplying 1.8.7 through either would need a historical baseline, overlay port or custom recipe to maintain. Enforced by `scripts/check_dependency_pins.py`; procedure in `docs/dependency-policy.md`. |
...
- **Still not started:** Reverb/live notifications, the dnd-kit/TanStack Query Homes rebuild, the versioned JSON layout API, and any actual cutover. Screenshot parity testing and the admin UI are **no longer** on this list. **Sentry has moved to a Phase 10 observability decision gate** — it is a dependency decision (no C++ SDK in the Ubuntu archive), not a missing feature, and `/metrics` is already implemented.
- **Phase 2b's remaining gap is one implementation item: the Compose frontend build service.** Everything else in its matrix is met — the build is warning-free under sanitizers, the preflight checks are recorded, the dependency strategy is decided and enforced, cutover and rollback are demonstrated, and CI was green on the head before the dependency-pinning commit (runs `35530042653`, `35530699981`). `frontend/dist` is still built out-of-band and bind-mounted. **CI must be green again on the current head before that is claimed.**
...
- **Phase 4's exit condition is met, having been wrongly claimed once.** The admin UI now drives every implemented `/api/admin/*` resource, including collectible editing — the operation whose absence made the first claim false. The CI steps that run the frontend build and browser suites failed on their first run for an environmental reason; that fix has since been re-observed as green (runs `35530042653` and `35530699981`), so the CI rows above are now evidence rather than intent. Phase 5 remains gated because Phase 1's OpenAPI document and the rest of Phase 2b are still outstanding.
