...
| CI covers TypeScript build and Playwright | **Yes — verified green.** `integration-smoke` builds the frontend, reloads the proxy, runs the admin UI suite on the runner's Chromium and the parity suite in the pinned container. All steps pass in runs `35530042653` and `35530699981`. (The first attempt failed because `frontend/dist` is a bind-mount source Docker auto-creates as **root**, so Vite's output-directory emptying hit EACCES; fixed by committing `frontend/dist/.gitignore` and reloading rather than restarting the proxy.) |
| **CI passes** | **GREEN.** Runs `35530042653` and `35530699981` on the current head: all three jobs success (`cpp-build-and-test`, `integration-smoke`, `cutover-check`), plus four consecutive green runs before them. The `77f7c87` readiness flake has not recurred. |
