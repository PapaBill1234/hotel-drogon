...
| CI covers TypeScript build and Playwright | **Yes — verified green.** `integration-smoke` builds the frontend, reloads the proxy, runs the admin UI suite on the runner's Chromium and the parity suite in the pinned container. All steps pass in run `35538279876` on the current head. (The first attempt failed because `frontend/dist` is a bind-mount source Docker auto-creates as **root**, so Vite's output-directory emptying hit EACCES; fixed by committing `frontend/dist/.gitignore` and reloading rather than restarting the proxy.) |
| **CI passes** | **GREEN.** Run `35538279876` on `cacc095`: all three jobs success (`cpp-build-and-test`, `integration-smoke`, `cutover-check`), plus runs `35530042653` and `35530699981` before it. Two earlier runs on `a09c9ef` failed at "Install system dependencies" because the runner image adds third-party apt repositories (`apt.postgresql.org`) that outrank Ubuntu's `libpq-dev` pin; fixed by making `release o=Ubuntu` authoritative at priority 1001 plus `--allow-downgrades`, and that fix is what the green run verifies. |
...
- **Phase 2b's remaining gap is one implementation item: the Compose frontend build service.** Everything else in its matrix is met — the build is warning-free under sanitizers, the preflight checks are recorded, the dependency strategy is decided, enforced and now green on CI, cutover and rollback are demonstrated (run `35538279876`, `cutover-check` job). `frontend/dist` is still built out-of-band and bind-mounted.
