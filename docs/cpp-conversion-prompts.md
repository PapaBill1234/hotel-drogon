...
5. Before implementing any feature, inspect the corresponding read-only legacy implementation under /legacy/phpretro-pdo as required by the plan. Port behavior, not PHP syntax. Never guess a PolarIS schema, hash format, permission, or emulator capability.
